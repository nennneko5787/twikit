...
class Endpoint:
    ...
    SEARCH_TIMELINE = url("cGK-Qeg1XJc2sZ6kgQw_Iw/SearchTimeline")
    ...
    USER_BY_SCREEN_NAME = url("AWbeRIdkLtqTRN7yL_H8yw/UserByScreenName")
    USER_BY_REST_ID = url("xavgLWWbFH8wm_8MQN8plQ/UserByRestId")
    ...
    RETWEETERS = url("tkd9_KXT2iXEshDFHrdfuw/Retweeters")
    FAVORITERS = url("oXXP-2dfybUgdp5iSp6dIg/Favoriters")
    ...
    FOLLOWERS = url("efNzdTpE-mkUcLARCd3RPQ/Followers")
    BLUE_VERIFIED_FOLLOWERS = url("YGl_IyrL0bFU7KHxQoSRVg/BlueVerifiedFollowers")
    FOLLOWERS_YOU_KNOW = url("dwvizjS4AcL6qn-i7Rgbdg/FollowersYouKnow")
    FOLLOWING = url("M3LO-sJg6BCWdEliN_C2fQ/Following")
    ...
    LIST_MANAGEMENT_PACE_TIMELINE = url(
        "47170qwZCt5aFo9cBwFoNA/ListsManagementPageTimeline"
    )
    ...
    COMMUNITIES_MAIN_PAGE_TIMELINE = url(
        "4-4iuIdaLPpmxKnA3mr2LA/CommunitiesMainPageTimeline"
    )
    ...
    MEMBERS_SLICE_TIMELINE_QUERY = url(
        "KDAssJ5lafCy-asH4wm1dw/membersSliceTimeline_Query"
    )
    MODERATORS_SLICE_TIMELINE_QUERY = url(
        "9KI_r8e-tgp3--N5SZYVjg/moderatorsSliceTimeline_Query"
    )
    ...
    GET_USERNAME_AVAILABILITY_AND_SUGGESTIONS = url(
        "1bMz-9lPrmIXrhFmXntTHw/GetUsernameAvailabilityAndSuggestions"
    )
...
class GQLClient:
    ...
    async def gql_get(
        ...
        **kwargs,
        ...
        return await self.base.get(
            url, params=flatten_params(params), headers=headers, **kwargs
        )
    ...
    async def similar_posts(self, tweet_id: str):
        ...
        return await self.gql_get(
            Endpoint.SIMILAR_POSTS, variables, SIMILAR_POSTS_FEATURES
        )
    ...
    async def tweet_detail(self, tweet_id, cursor):
        ...
        params = {"fieldToggles": {"withAuxiliaryUserLabels": False}}
        return await self.gql_get(
            Endpoint.TWEET_DETAIL, variables, FEATURES, extra_params=params
        )
    ...
    async def retweeters(self, tweet_id, count, cursor):
        return await self.tweet_engagements(
    ...
    async def bird_watch_one_note(self, note_id):
        ...
        return await self.gql_get(
            Endpoint.FETCH_COMMUNITY_NOTE, variables, COMMUNITY_NOTE_FEATURES
        )
    ...
    async def user_tweets_and_replies(self, user_id, count, cursor):
        return await self._get_user_tweets(
            user_id, count, cursor, Endpoint.USER_TWEETS_AND_REPLIES
        )
    ...
    async def user_creator_subscriptions(self, user_id, count, cursor):
        return await self._friendships(
            user_id, count, Endpoint.USER_CREATOR_SUBSCRIPTIONS, cursor
        )
    ...
    async def user_dm_reaction_mutation_add_mutation(
        ...
        return await self.gql_post(
            Endpoint.USER_DM_REACTION_MUTATION_ADD_MUTATION, variables
        )
    ...
    async def user_dm_reaction_mutation_remove_mutation(
        ...
        return await self.gql_post(
            Endpoint.USER_DM_REACTION_MUTATION_REMOVE_MUTATION, variables
        )
    ...
    async def delete_list_banner(self, list_id):
        ...
        return await self.gql_post(
            Endpoint.DELETE_LIST_BANNER, variables, LIST_FEATURES
        )
    ...
    async def list_remove_member(self, list_id, user_id):
        variables = {"listId": list_id, "userId": user_id}
        return await self.gql_post(
            Endpoint.LIST_REMOVE_MEMBER, variables, LIST_FEATURES
        )
    ...
    async def list_management_pace_timeline(self, count, cursor):
        ...
        if cursor is not None:
            variables["cursor"] = cursor
        return await self.gql_get(
            Endpoint.LIST_MANAGEMENT_PACE_TIMELINE, variables, FEATURES
        )
    ...
    async def list_latest_tweets_timeline(self, list_id, count, cursor):
        ...
        if cursor is not None:
            variables["cursor"] = cursor
        return await self.gql_get(
            Endpoint.LIST_LATEST_TWEETS_TIMELINE, variables, FEATURES
        )
    ...
    async def join_community(self, community_id):
        variables = {"communityId": community_id}
        return await self.gql_post(
            Endpoint.JOIN_COMMUNITY, variables, JOIN_COMMUNITY_FEATURES
        )

    async def leave_community(self, community_id):
        variables = {"communityId": community_id}
        return await self.gql_post(
            Endpoint.LEAVE_COMMUNITY, variables, JOIN_COMMUNITY_FEATURES
        )
    ...
    async def get_username_availability_and_suggestions(
        self, username: str, *, include_suggestions: bool
    ):
        variables = {
        ...
        return await self.gql_post(
            Endpoint.GET_USERNAME_AVAILABILITY_AND_SUGGESTIONS,
            variables,
        )
