...
class Client:
    ...
    def _build_report_tweet_new_payload(
        self, *, flow_id: str, tweet_id: str, user_id: str, screen_name: str
    ):
        return {
            "additional_query_params": {},
            "input_flow_data": {
                "requested_variant": '{"client_app_id":"3033300","client_location":"tweet::result","client_referer":"/'
                + screen_name
                + "/status/"
                + tweet_id
                + '","is_media":true,"is_promoted":false,"report_flow_id":"'
                + flow_id
                + '","reported_tweet_id":"'
                + tweet_id
                + '","reported_user_id":"'
    ...
    def _build_report_tweet_payload(self, *, flow_token: str, value: str):
        return {
    ...
    async def report_tweet(
        ...
        response, _ = await self.v11.report_tweet_flow(payload, new=True)

        payload = self._build_report_tweet_payload(
            flow_token=response["flow_token"], value=category
        )
        response, _ = await self.v11.report_tweet_flow(payload, new=False)

        if response["status"] == "success":
            return True

        payload = self._build_report_tweet_payload(
            flow_token=response["flow_token"], value=sub_category
        )
        response, _ = await self.v11.report_tweet_flow(payload, new=False)
